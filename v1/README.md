# Jarvis Legal Agent — v1 / Local Qwen

Первая публичная реконструкция юридического AI-ассистента. Основной агент работает локально на
`qwen3:14b` через Ollama, а изображения и PDF-сканы распознаются моделью `qwen3-vl:4b`.
Репозиторий использует только синтетические данные.
