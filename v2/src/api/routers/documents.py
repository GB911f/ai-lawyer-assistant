...
from src.document_analyzer.service import summarize
...
async def content(path: str = Query(...), user: User = Depends(current_user)) -> dict:
    ...
    parsed = parse_file(document_path)
    return {
...
async def document_summary(path: str = Query(...), user: User = Depends(current_user)) -> dict:
    ...
    return {
        ...
        "model": "deterministic-demo-analyzer",
        "status": "done",
        "analyzed_at": None,
        "error": None,
    }
...
async def deep_analyze(path: str = Query(...), user: User = Depends(current_user)) -> EventSourceResponse:
    document_path = safe_demo_path(path)
    parsed = parse_file(document_path)
    analysis = summarize(parsed.text)
    ...
    async def events():
        ...
        yield {"event": "text", "data": json.dumps({"delta": f"## Глубокая аналитика\n\n{analysis}"}, ensure_ascii=False)}
...
async def analyze_upload(
    ...
    return {"file_name": file.filename, "document_text": parsed.text, "answer": summarize(parsed.text), "citations": []}
